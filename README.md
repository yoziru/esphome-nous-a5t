...
If the upload reaches 100% then fails with `ERROR receiving update end result: Finishing update failed`, the firmware target doesn't match the physical chip — this firmware only supports the Nous A5T (ESP8266/ESP8285), so don't flash it onto other hardware.
